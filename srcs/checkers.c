...
/*   Updated: 2023/10/15 00:53:15 by aaires-b         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int checker_path()
{
	...
	i = 0;
	newmap = create_map_cpy(engine()->map.map_grid); // falta create
	floodfill(engine()->map.map_grid, engine()->map.p_pos_x, engine()->map.p_pos_y);
	free_loop(newmap);
	if(data_carct() == 1) // falta data caract
		return(1);
	return(0);
}
